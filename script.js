...
async function loadData() {
  ...
  try {
    ...
    for (let trans of state.transactions) {
      await db.collection("transactions").doc(trans.id.toString()).set({
        ...trans,
    ...
    for (let goal of state.goals) {
      await db.collection("goals").doc(goal.id.toString()).set({
        ...goal,
  ...
  if (localStorage.getItem("isLoggedIn") === "true") {
    openModal('transaction'); // ✅ logged in → open modal
  } else {
    window.location.href = "login.html"; // ✅ not logged in → login page
  }
});
